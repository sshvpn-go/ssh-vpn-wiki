...
> **关于 TUN 模式的系统权限说明**
> 为了让系统级流量走指定的远程接入路径，ssh-vpn 会新建一个虚拟网卡 (TUN Device)，并按需调整系统路由，同时监听 `127.0.0.1:53` 以配合 DNS 解析流程。
...
为了在远程接入时兼顾本地资源与内部系统访问，ssh-vpn 客户端内置了智能分流引擎。您可以让局域网、内网地址或指定域名直连，同时将需要通过 SSH 隧道访问的目标交给对应规则处理。
